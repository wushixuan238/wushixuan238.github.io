      ...
      <div className="flex gap-16">
        {skillCategories.map((category) => (
          <div className="flex flex-col gap-2.5" key={category.category}>
            <p className="font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-widest text-[11px]">
              {category.category}
            </p>
            <div className="flex flex-col gap-1">
              {category.skills.map((skill) => (
                <p key={skill} className="text-[13px] text-neutral-700 dark:text-neutral-300">{skill}</p>
              ))}
            </div>
          </div>
        ))}
